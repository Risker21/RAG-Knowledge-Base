...
| **纯内存向量库** | 不依赖第三方向量数据库，启动时从 MySQL 加载，支持向量范数预计算优化检索性能 |
| **Redis 分布式会话** | 支持多实例部署的会话管理 |
| **Embedding 缓存** | Redis 缓存已计算的嵌入向量，减少重复 API 调用 |
| **SSE 批量发送** | 缓冲批量发送 token，减少序列化和网络开销 |
...
| 嵌入模型 | `app.openai.embedding-model` | `doubao-embedding-large` | 文本向量化模型 |
...
| Embedding 缓存过期 | `app.embedding.cache-ttl-hours` | `24` | 嵌入向量缓存时间（小时） |
| SSE 连接超时 | `app.sse.timeout-ms` | `300000` | SSE 长连接超时（毫秒） |
| SSE 批量发送阈值 | `app.sse.batch-size` | `50` | SSE 缓冲发送的字符数阈值 |
...
> **测试环境**：4 核 CPU / 8GB 内存 / 国内网络（火山引擎 API 延迟 ~150ms）

| 指标 | 数值 | 说明 |
|------|------|------|
| **知识库规模** | 单知识库支持 **10,000+ 文档** | 基于内存向量库，受 JVM 堆内存限制；每文档平均切分为 ~10 个 chunk，单 KB 可容纳 **100,000+ 向量** |
| **向量维度** | 2560（取决于模型） | 使用 `doubao-embedding-large` 文本向量模型，维度为 2560 |
...
| **LLM 首 token 延迟** | **1-3s**（SSE 流式） | 从提问到前端显示第一个字符的时间 |
| **LLM 完整响应时间** | **3-12s**（取决于回答长度） | 平均回答长度 300-500 字，生成速度约 40-60 tokens/s |
| **SSE 优势** | **前端感知延迟降低 60-80%** | 同步模式需等待完整响应（3-12s），流式模式首字符 1-3s 即可呈现 |
| **检索召回率** | **~85-92%**（标准数据集测试） | 使用余弦相似度 + 阈值过滤，topK=5 时的相关片段召回率 |
| **检索准确率** | **~75-85%**（人工评估） | 基于文档内容回答问题的正确率，未找到相关信息时明确提示 |
| **并发能力** | **支持 100+ 并发对话** | SSE 长连接 + 异步处理，受 AI API 配额限制 |
| **文档处理速度** | **~500 字符/秒**（含 embedding） | 1000 字文档完整处理约需 2-3 秒 |

### 性能优化策略

| 优化项 | 实现方式 | 效果 |
|--------|----------|------|
| **向量范数预计算** | `VectorStore` 存储时计算并缓存向量 L2 范数 | 检索时避免重复计算，提升约 20% 性能 |
| **PriorityQueue 检索** | 使用最小堆维护 Top-K 结果 | 时间复杂度从 O(n log n) 优化为 O(n log k)，100K 向量检索提升 30-50% |
...
- **大规模场景**：当前内存向量库适合中小规模（100K 向量以内），如需支持百万级向量，建议接入 Milvus/Pinecone/Weaviate 等专业向量数据库
- **性能监控**：可添加 Prometheus + Grafana 监控，跟踪 embedding 耗时、检索耗时、LLM 响应时间等关键指标
- **缓存策略**：当前已实现 Embedding 缓存，可进一步对高频问题的完整回答进行 Redis 缓存
...
用户输入问题
     ↓
[1] EmbeddingService.embed(问题)          ← POST /api/v3/embeddings
     ↓
