...
async function handleRegister() {
  ...
  loading.value = true
  try {
    const res = await userStore.doRegister(username.value, password.value, confirmPassword.value, captcha.value)
    if (res.code === 200) {
      router.push('/kb')
    } else {
...
<template>
  ...
  <div class="auth-container">
    <div class="auth-card">
      ...
      <p class="auth-subtitle">创建你的墨韵账号</p>
      ...
      <form @submit.prevent="handleRegister" class="auth-form">
        ...
        <div class="form-group">
          ...
          <input class="form-input" v-model="confirmPassword" type="password" required placeholder="再次输入密码" />
        </div>
        <div class="form-group">
        ...
        <button type="submit" class="btn btn-accent btn-full" :disabled="loading">{{ loading ? '注册中...' : '注 册' }}</button>
      </form>
      <p class="auth-link">已有账号？<RouterLink to="/login">立即登录</RouterLink></p>
    </div>
  </div>
...
.error-msg { color: var(--cinnabar); font-size: 13px; margin-bottom: 16px; text-align: center; }
.btn-accent {
...
.auth-link { margin-top: 24px; font-size: 13px; color: var(--ink-muted); }
.mountain-scene {
  position: fixed;
